...
plot_cum <- ggplot(data = medail_cum, aes(x = year, y = med_cum, group=abb, color=abb)) +
  ...
  view_follow(fixed_x = TRUE, 
              fixed_y = TRUE) +
  transition_reveal(year)
plot_cum <- animate(plot_cum , end_pause = 10)
plot_cum 
...
animacion <- medail_cum1 %>%
  ...
  ggtitle("Nombre de médailles cumullées entre 1980 et 2016") +
...
plot3 <- animate(animacion,fps = 25, duration = 15, end_pause = 10,rewind = TRUE)
plot3
```
