...
Par exemple, comparons deux courses: une avec la vitesses moyenns la plus basse et l'autre - la plus élevée.

```{r, warning=FALSE}
compare <- speed %>% filter(mean_speed == min(mean_speed) | mean_speed == max(mean_speed))
compare %>% 
  pivot_longer(cols = c(mean_speed, men_pct, mean_age, distance, elevation_gain, aid_stations), 
...
compare %>% 
  pivot_longer(cols = c(mean_speed, men_pct, mean_age, distance, elevation_gain, aid_stations), 
               names_to = "measurement", values_to = "value") %>% 
  filter(measurement != "elevation_gain") %>% 
...
compare %>% 
  ...
  ggplot(aes(x = race, y = value, fill = race)) +
  geom_col() +
  ggtitle("Elevation_gain") +
  scale_fill_brewer(palette = "green") +
  theme_minimal()
```

Comme on peut le voir sur le tableau comparatif et les graphiques, dans la course avec la vitesse moyenne la plus élevée, la distance, le niveau de dénivelé et l'âge moyen des participants étaient plus faibles, tandis que le nombre de stations d'assistance était plus grand, ce qui explique assez logiquement la différence de vitesse des coureurs. Dans la plus grande mesure, ces deux courses diffèrent par le nombre de mètres de l'élévation et l'âge des participants : ces paramètres sont très probablement déterminants de la vitesse moyenne des gagnant. 
