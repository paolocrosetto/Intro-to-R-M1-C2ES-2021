...
À partir de ces graphiques, on peut voir la tendance générale selon laquelle les femmes participant aux les ultra trails sont, en moyenne, légèrement plus jeunes que les hommes. Cette différence est plus visible parmi les gagnants, surtout dans la zone de 39-59 ans. Pourtant, en general, la distribution des âges des femmes et des hommes n'est pas très différente.
...
Bien, mais cela aurait pu prendre ~10 lignes de code:

```{r}
utr %>% 
  select(race,participation) %>% 
  mutate(participation = case_when(participation == "solo" ~ "Solo",
                                   participation == "solo" ~ "Solo",
  ...
  ggplot(aes(x = participation, y = n)) +
  geom_col(fill = c("light green", "dark green")) +
  geom_text(aes(label = n, vjust = -0.5), color = "black", size = 4) +
  ggtitle("Nombre de trails individuelles et par équipes") +
  theme_minimal()
