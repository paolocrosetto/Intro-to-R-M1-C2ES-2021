...
```
Cette valeur ne dit rien sur une relation causale. Il montre seulement que les domaines dans lesquels il y a plus de femmes connaissent actuellement une baisse du nombre de doctorats.
...
sex_and_race %>% 
  ...
  pivot_longer(-c(Ethnicity_visa, Sexe, lvl), names_to = 'year', values_to = 'n_phds') %>% ggplot(aes(x=as.numeric(year), n_phds, color = Ethnicity_visa)) + geom_line(size = 1) +
                 theme_minimal() + labs(x = 'year', color = 'Ethnicity') +
  scale_x_continuous(breaks= pretty_breaks()) 
