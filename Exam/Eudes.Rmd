...
ss %>% 
  ...
  theme(plot.title = element_text(hjust = 0.5))
```
La pop domine en populairté pour l'ensemble du jeu de données. On peut également observer que ledm et le rnb sont en moyenne moins écoutées.
...
ggplot(ss,aes(x = dureem,y = track_popularity,color=playlist_genre))+
  ...
  theme(plot.title = element_text(hjust = 0.5))
```
L'EDM se démarque et dépend un peu plus de la durée que les autres genres, mais la relation qu'on les genres entre la durée du titre et sa popularité est plutot homogène. Il est preferable de faire une chanson entre 2 minutes 30 et 3 minutes 45 pour qu'elle fonctionne.
...
```
On constate que les chansons les plus populaires sont les plus récentes, avec seulement deux chansonsde 3 ans ou plus: "Believer" (2017), et "All I want for Christmas is You" (1994) dont la popularité est peut être expliquée par la date de prélèvement des données proche de la période de Noël.
...
```
Les chansons les plus populaires sont celles sorties 3 mois avant. On observe que les tubes sortis en juin sont restées populaires.
