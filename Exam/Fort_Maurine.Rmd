...
#![HOTEL](E:/ETUDES/MASTER BDA/LOGICIEL SPECIALISE/CM3/Student Area/Maurine/hotel.jpg)
...
```{r, message=FALSE}
hotels %>% 
  select(hotel, date, arrival_date_month) %>% 
  group_by(date, arrival_date_month, hotel) %>% 
  summarise(N = n()) %>% 
  ggplot(aes(x = date, y = N, fill = hotel)) + 
  geom_col(show.legend = FALSE) +
  facet_wrap(~hotel) +
...
```
> <p align="justify" >
Dans un premier temps, si nous observons les variations durant une année, soit de juillet 2015 à juillet 2016, nous remarquons que ces dernières se répètent l'année suivante.  
Dans un second temps, les deux graphiques ci-dessus présentent la même tendance. En effet, que ce soit dans le City Hotel ou dans le Resort Hotel, les clients réservent plutôt des séjours durant les saisons estivales, comme le printemps ou l'été. 
</p>
...
```{r, message=FALSE}
ancienneannulation <- with(hotels, ifelse(previous_cancellations == 1,1,0))
annulation <- with(hotels, ifelse(is_canceled == 1,1,0))

cor.test(ancienneannulation, annulation, use = "complete.obs")
```
> <p align="justify" >
Nous souhaitons savoir si le fait d'avoir déjà annulé un séjour influence le fait d'annuler de futurs séjours. 
Nous trouvons une corrélation égale à 0.274 ce qui signifie que les variables testées sont faiblement corrélées.
...
df <- prop.table(table(hotels$reservation_status, hotels$hotel))*100
...
```
> <p align="justify" >
Ce graphique nous informe, qu'à échelle égale, la part d'annulation en City Hotel est plus importante que celle du Resort Hotel. 
Cependant, de façon globale, nous observons la même tendance pour les deux hôtels.
</p>
...
```
> <p align="justify" >
Les clients ayant des séjours de courte durée choisissent principalement la pension complète. 
Dans les City Hotel, les clients ayant des séjours de longue durée choisissent soit la formule "bed & breakfast" ou ils ne prennent simplement pas de repas.
...
Pour les séjours de courte durée, que ce soit dans un City Hotel ou un Resort Hotel, les clients choisissent majoritairement la formule "full board" donc la pension complète.
</p>
...
hotels %>%
  ...
  geom_col() +
  labs(x = "Types de chambre", y = "Nombre de réservation", title = "Nombre de réservations par type d'hotel en fonction des mois" ) +
  theme_minimal() +
  theme(legend.title = element_blank(), plot.title = element_text(size = 12, face = "bold", hjust = 0.5),
        strip.background = element_rect(colour = "black", fill = "white", size = 1.5, linetype = "solid"),
        axis.title.x = element_text(size = 10),
...
```{r, message=FALSE}
hotels %>% 
  select(adr, arrival_date_year, stays_in_weekend_nights, stays_in_week_nights, hotel) %>% 
  mutate(temps = stays_in_weekend_nights + stays_in_week_nights) %>% 
  sample_n(6000) %>% 
  drop_na(adr, hotel) %>% 
  ggplot(aes(x = temps, y = adr, color = hotel)) +
  geom_point() +
...
```
> <p align="justify" >
Nous choisissons un échantillon de 5% aléatoirement choisit dans nos données pour pouvoir faire une bonne représentation visuelle. 
Nous observons que plus le séjour est long et plus le tarif journalier moyen tend à diminuer. 
</p>
...
> <p align="justify" >
 ...
 > <p align="justify">
- Les clients qui réservent des séjours de courte durée choisissent majoritairement la pension complète ou la demi-pension. </p>
> <p align="justify">
