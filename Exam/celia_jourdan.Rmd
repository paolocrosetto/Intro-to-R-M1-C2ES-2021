...
energy_types %>% 
  ...
  geom_col()+
  facet_wrap(~country)+
  scale_fill_manual(values = c("orange", "brown", "yellow", "grey", "green", "red", "blue", "black"))+
  coord_polar(theta = "y")+
  theme_void()+
...
energy_types$country_name[273:280] <- "Bosnia and Herzegovina"
energy_types$country_name[17:24] <-"Czech Republic"
...
q3 <- country_totals %>% 
  ...
  left_join(energy_types, by =  "country_name") %>% 
  select(country_name, type, `2018`, exp_net)
...
Les graphiques confirment que c'est pour le nucléaire que la relation croissante entre la production d'un type d'énergie en particulier et les exportations nettes est la plus sgnificative significative.
