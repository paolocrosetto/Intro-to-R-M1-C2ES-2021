...
Bien. Toutefois, je ne comprends pas non plus pourqui faire une base de données séparée pour les dates de naissance, où on perd toute autre information pour créer la variable de l'âge au moment du prix avec le `mutate`; vous auriez pu faire cela dans une seule chaine et vous passer de pipe. birth_date est déjà en format DATE dans les données originelles...

```{r}
nobel_no_doublon %>% 
  mutate(age = unlist(prize_year - date_naissance)) %>% 
  # à partir d'ici c'est votre code exact
  ggplot(aes(x=prize_year, y = age))+
  geom_point()+
  geom_smooth()+
  facet_wrap(~category)+
  labs(title = "Evolution de l'âge des gagnants par discipline entre 1901 et 2016" )+
