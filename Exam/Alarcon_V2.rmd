...
Il y a une méthode assez simple, la voilà, pour adultes/enfants. `case_when` permet de créer des variables catégorielles à partir de "cas", et dans ce cas, de changer les catégories d'une var catégorielle:

```{r}
african_names %>% 
  mutate(adult = case_when(gender == "Man" | gender == "Woman" ~ "Adulte",
                           gender == "Boy" | gender == "Girl"  ~ "Enfant",
                           is.na(gender)                       ~ "NA")) %>% 
  group_by(adult) %>% 
  tally() # c'est équivalent de "count"
...
J'ai donc fait ici, un Dumbbell pour l'année 1850 vs 1870 (car pas assez de données pour l'année 1790, en effet nous n'avons ni le midwest ni l'ouest, ça aurait été très interressant avec cette année, mais nous n'aurions pas pu comparer comme avec l'année 1850). Pour vous montrer qu'il manque le midwest et l'ouest, je vous montre le tableau (1er et seul tableau apparaissant). 
Je n'aurais pas utilisé ce plot, car ce n'est pas très lisible avec mes données, au vu des échelles, néanmoins il est très joli. 
Le Dummbbel pour les esclaves noirs est le 3ème, j'ai aussi fait les autres pour pouvoir faire des comparaisons. Surtout celui de la part des personnes noires libres, car il montre l'inverse de celui des peronnes noires esclaves. 
...
Très bien, mais je ne comprends pas pourquoi vous avez finalement utilisé des outils autres que ceux du cours. C'est très bien d'explorer et d'essayer et il y a toujours plusieurs façon en R de faire les choses -- tant que ça marche! -- mais cela aurait été beaucoup plus court avec ce qu'on a vu en cours.

En plus, la question demandait juste la part de noirs esclaves, donc vous auriez pu vous concentrer sur cela. C'est bien d'en faire plus, mais cela risque de noyer le poisson!

Par exemple, pour sélectionner/créer des variables:

```{r}
plot_this <- census %>% 
  filter(year == 1850 | year == 1870,
         region != "USA Total",
         is.na(division)) %>% 
  mutate(free = round(100*black_free/total,2),
         slaves = round(100*black_slaves/total, 2)) %>% 
  select(region, year, free, slaves) 
...
bien; encore une fois, on pouvait faire plus vite et plus compact. En général, utilisez plus le pipe %>% et évitez de créer un objet à chaque calcul (cela pollue votre environnement pour rien) et si possible mettez une instructions par ligne (aide avec le debug). Surtout pour les plots, c'est du code bcp plus lisible!

Comme ça:

```{r}
african_names %>%  
  group_by(port_embark) %>% 
  tally() %>% 
  mutate(part = n/sum(n)) %>% 
  arrange(-part) %>% 
  top_n(5) 
