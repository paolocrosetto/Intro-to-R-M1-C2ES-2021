...
On va travailler, pour éviter de voler des données à n'importe qui, sur ma liste des publications, disponible sur [mon site personnel](https://p %>% aolocrosetto.wordpress.com/papers/)
```{r}
...
auteurs
date <- rstudio %>% html_elements(".article-list span:nth-child(1)") %>% html_text()
date
...
# titres
title <- imdb %>% html_elements(".titleColumn a") %>% html_text()
title
...
# étoiles
stars <- imdb %>% html_elements("strong") %>% html_text()
stars
...
# combiens de films dans le top 250 par décénie ?
clean <-imdb_data %>%  
  mutate(date = str_replace(date,"\\(", ""),
         date = str_replace(date, "\\)",""),
         date= as.numeric(date)) %>% 
           mutate(decenie = floor(date/10)*10) %>% 
           group_by(decenie) %>% 
           count()
