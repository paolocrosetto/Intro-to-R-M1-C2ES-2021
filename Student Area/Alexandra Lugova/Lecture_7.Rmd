...
# recap - telecharger un table de web en r
...
df %>% 
  select(Country, GDPpc = 'GDP per capita (Int$)[1]') %>% 
  mutate(GDPpc = str_replace('GDPpc', ',', '')) %>% 
  GDPpc = as.numeric(GDPpc)) %>% 
  ggplot(aes(y = reorder(Country, GDPpc), x = GDPpc)) +
  geom_col()
```
...
got_list <-  got %>% html_elements(".wikitable") 
...
```{r, message= FALSE}
titres <- rstudio %>%  html_element("h1 a") %>%  html_text()

auteurs <- rstudio %>%  html_element("span + span") %>%  html_text()

dates <- rstudio %>%  html_element(".article-list span:nth-child(1)") %>%  html_text()

summary <- rstudio %>%  html_element(".summary") %>%  html_text()
```
...
# page [0-9]{1-2} - pages 0-19 (1 ou 2 chiffres avec 0-9)
