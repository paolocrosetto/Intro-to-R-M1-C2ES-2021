...
head(airquality)
```
## lineaire modele (regression lineaire)

```{r ex1}
lm(Ozone ~ Temp + Wind + Solar.R, data = airquality)

# airquality %>%  lm(Ozone ~ Nemp + Wind + Solar.R, data = .)
```
...
> Nos résultats sont maintenant en forme de data.frame et peuvent donc être utilisés pour nos analyses, plots. Faites un plot des coefficients de la régression avec des barres d'erreur
...
```{r}
reg1 <- lm(Ozone ~ Wind, data = airquality) 
reg1 %>% tidy
```
...
airquality %>% 
  ggplot(aes(Wind, Ozone)) +
  geom_point() +
  geom_smooth(method = 'lm', color = "purple") +   # cela lance une regression lineaire
  theme_minimal()
```
...
Comment l'espérance de vie a-t-elle évolué dans le temps pour tous les pays? un ggplot
...
Pour répondre à cela il faut faire une régression par an, et après regarder les coefficients et leurs intervaux de confiance pour voir si l'effet est toujours bien vivant ou il s'affaiblit. De plus, le faire par continent aiderait. Peut-on faire cela?
...
df %>% 
  group_by(country) %>% 
  group_modify(~tidy(cor.test(.$lifeExp, .$gdpPercap))) %>% 
  filter(country == 'France' | country == 'Japon' | country == "Bolivia") %>% 
