...
Si on peut passer directement par la base, pas besoin de passer par le back-end
...
df %>% 
  ...
  separate(croissance, into = c("croissance","percent"), sep = -1) %>% 
  mutate(croissance = as.numeric(croissance)) %>% 
  ggplot(aes(y=reorder(State, croissance), x = croissance))+
  geom_col()
  
```
Probleme ici c'est que percent change est en caractère. 
...
A RECUPERERRRRRR SUR LE DOC 

# plotting
df3 %>% 
   ggplot(aes(y=reorder(Episode number, GDP), x = GDP))+
  geom_col()
...
auteurs <- rstudio %>%  html_elements("spa n+ span") %>% 
  html_text()
...
titres <- imbd %>% html_elements(".titleColumn a") %>%  html_text()

# date

dates <- imbd %>% html_elements(".secondaryInfo") %>% html_text()

# étoiles

etoiles <- imbd %>% html_elements("strong") %>%  html_text()


#3. coller les vecteurs ensemble avec "tibble()"

imbd <- c(titres, dates, etoiles)
...
## J'ulitse le filtre (crawlUrl.. -> je prends la page de 1 à 2)
```


J'utilise le filtre (crawlUrl.. -> je prends la page de 1 à 2)
