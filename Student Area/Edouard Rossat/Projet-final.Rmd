...
library(broom) #Permet d'utiliser la fonction tidy, créant un dataframe avec un test (ici cor)
...
```{r,warning=FALSE,message=FALSE,error=FALSE}
rando<- readPNG("rando.png", native = TRUE)
...
#On plot ce vecteur des corrélations sans rating   
  ...
  coord_flip()  + labs(title = "Corrélation entre la note et les caractéristiques") +ylab("Corrélations") + xlab("Caractéristiques") +  theme(plot.title = element_text(face = "bold",size = 14,hjust = 1)) + theme_minimal() 
```


##### **9. Quelles sont les distances idéales pour les randonnées pour enfant ?**
```{r}
...
kids<- readPNG("kidshike.png", native = TRUE)
...
#On affiche les randonnées pour les enfannts ayant une note > 2.5 associé à la distance
#à parcourir en km
hike_dataft %>% filter(features_Goodforkids == 1,!is.na(length_km),rating>2.5) %>% select(rating,length_km) %>% ggplot() + geom_line(aes(x=rating,y=length_km),color="goldenrod4") + geom_point(aes(x=rating,y=length_km),size = 1,color ="khaki4") + 
  geom_hline(yintercept = m_km$m_km_k,color="darkgreen",linetype = "dashed") + labs(title = "Nuage de points des distances idéales des randonnées pour enfant") +ylab("distance en km") + xlab("notes") +  theme(plot.title = element_text(face = "bold",size = 14)) + scale_y_continuous(breaks = seq(0,40,2.5)) +theme_minimal() + 
