...
#je joins les deux tableaux : c'est possible car j'ai ma clé étrangère, laureate_id, qui me permet de relier les deux tableau et j'ai autant de lignes.
publication <- publi %>% 
...
publication %>% 
  ...
  facet_wrap(~category, scales = "free") +
  theme_classic()+
  labs(title = "Evolution du nombre de publications écrites par les gagnants par discipline" )+
  theme(plot.title = element_text(hjust = 0.5))

```
```{r}
 #moyenne pour ceux ou ya plusieurs personnes pour un prix pour une meme année et meme category ca signfie que le prix a ete partagé

  publication %>% 
  group_by(prize_year, Nombre_publication, category) %>% 
  summarise(mean_publi = mean(Nombre_publication)) %>% 
  ggplot()+
  aes(x = prize_year, y = Nombre_publication, fill = mean_publi)+
  geom_point()+
  geom_smooth()+
  facet_wrap(~category, scales = "free")  +
  theme_classic()+
...
#non fini ca n'a pass prit en compte la moyenne!!!
...
publication %>% 
  ...
  summarise(mean_pub = mean(Nombre_publication)) %>% 
  ggplot(aes(x = mean_pub, y=Nombre_publication))+
  geom_point()+
  geom_smooth()
  ...
  publication %>% 
  group_by(prize_year, Nombre_publication) %>% 
  summarise(mean_publi = mean(Nombre_publication)) %>% 
  ggplot()+
  aes(x = prize_year, y = Nombre_publication, fill = mean_publi)+
  geom_point()
