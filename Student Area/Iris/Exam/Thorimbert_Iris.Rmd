...
Dans un soucis de simplification de la base de données et de son analyse, j'ai fait le choix de supprimer toutes les données. Tout d'abord toutes celles rattachées au prix nobel de la paix. En effet, j'obtiens de nombreuses valeurs manquantes et de plus, le fait que des organisations ont parfois été récompensées, rend notre analyse plus complexe.

```{r}
# nettoyage de la base de données

nobel <- nobel_winners %>% 
  .[-13] %>%  # suppression colonne 13
  .[!nobel_winners$category=="Peace",] # suppression tous les prix associés à peace

nobel_no_doublon <- unique(nobel[ 1:839,])
```

A partir de maintenant nous avons 817 observation et 17 variables.
...
### **Quels est l'évolution du**     
, prize_year>1985 # dans le filter 
```{r}
...
Cependant n'est pas totalement correct car nous prenons ici le pays de naissance du lauréat, or certain individu ont, entre temps, changer de nationnalité. Nous pouvons par exemple prendre Marie Curie, né en Pologne puis naturalisée française après son mariage.  
De plus, comme notre base de donnée commence au début du 20ème siècle, nous avons des pays qui aujoud'hui n'existe plus.
```{r}
nobel_no_doublon %>% 
  select(birth_country) %>% 
