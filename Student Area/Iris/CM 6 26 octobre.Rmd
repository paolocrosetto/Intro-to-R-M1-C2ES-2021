...
```{r}
head(airquality)# resultat n'apparait pas donc taper dans la console pour le voir ou l'enregistrer dans une variable
airquality
```


```{r ex1}
# une régression s'écrit Y = cte+B1*VAR+B2* VAR...
...
```{r}
coef(firstreg)  # sous version de summary
firstreg$coefficients #on peut recup les coeff avec le $
```
...
```{r}
exercice1 <- lm(Ozone ~ Wind, data = airquality)
summary(exercice1)

# tidyverse
# on peut utiliser le pipe et ca evite d'enregistrer dans un element
# meme résultats
airquality %>% 
  lm(Ozone ~ Wind, data = .) %>% 
  summary()
...
```{r}
library(broom)
tidy(firstreg) # resultat n'apparait pas donc taper dans la console pour le voir
...
reg %>%  augment() # resultat n'apparait pas donc taper dans la console pour le voir
airquality
```
...
plot1 +
  geom_line(aes(x = Temp, y = .fitted), color = "green")+
  geom_segment(aes(x = Temp, xend = Temp, y = Ozone, yend = .fitted), color = "grey") # afin de faire disparaitre le segment qu'on voit au milieu du point il faut juste inverser l'ordre car R superpose les etapes
...
airquality %>% 
  ggplot(aes(Wind,Ozone))+
  geom_point()+
  geom_smooth(method = "lm") # cela lance une regression lineaire comme celle faite ci-dessus
```
...
```{r}
reg2 %>% glance() # resulrtat n'apparait pas donc taper dans la console
```
...
df %>% 
  ...
  tidy() %>%  # resulats a voir dans la console
  summary()

# pvaleur 0 donc significatif donc plus on est riche plus l'esperence est eleve
```
...
pourquoi le fit est si mauvais? parce que les données contiennent une observation par pays **par an** et dans ce plot on ne tient pas en compte cela. (données non indépendantes : on ne tient pas compte de l'évolution par an)
...
# au lieu de faire un seul modele, il en a fait pour chaque groupe année

year_cont # n'apparait pas donc a taper dans la console
```
...
```{r}
year_cont %>% 
  filter(term != "(Intercept)") %>%  # on enleve (Intercept)
  ggplot(aes(year, estimate, color = continent))+
  geom_point()+
  facet_wrap(~continent, scales = "free") # l'echelle est adapté par rapport a ses données d'ou le scales = free
  
# Amerique et europe il y a une relation entre la richesse et l'esperance de vie mais elle est decroissante au fur et a mesure des années
...
# resultats a regarder dans la console
# en asie chaque année ya 1M3 en plus, europe 102 mille...
...
```{r}
cor(df$lifeExp,df$gdpPercap) # correlation positive, si tu es plus riche, tu vis plus longtemps
```

par contre si on veut tester la significativité statistique de cette corrélation il faut utiliser `cor.test()`:

```{r}
cc<- cor.test(df$lifeExp,df$gdpPercap) # la correlation est postive et p valeur vraiment petite
cc
```
...
df %>% 
  group_by(continent) %>% 
  summarise(correlation = cor(lifeExp,gdpPercap)) # resultat dans la console
...
df %>% 
  group_by(continent) %>% 
  group_modify(~tidy(cor.test(.$lifeExp,.$gdpPercap))) # resultat à voir dans la console
...
le test T à une variable sert à tester si la moyenne d'une variable est significativement différente d'une valeur de référence (par défaut: 0); à deux variables à tester si la moyenne d'une variable est statistiquement différente de l'autre. 
...
df %>% 
  ...
  summarise(moyenne_lifeExp = mean(lifeExp)) # resultat dans la console
```
...
df %>% 
  group_by(continent, year) %>% 
  group_modify(~tidy(t.test(.$lifeExp, mu = 60, alternative = "greater"))) # resultat console
...
df %>% 
  ...
  geom_line(yintercept = 12000, color="green", linetype ="dotted")
