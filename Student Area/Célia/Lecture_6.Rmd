...
library(tidyverse)
library(broom) #prend le résultat de tous les outils statistiques et les met en format utilisable en tidyverse
...
```{r ex1}
# lm = linear model. Une régression s'écrit normalement Y = K + b1*VAR + b2*VAR ...
# En R c'est Y ~VAR1 + VAR2
lm(Ozone ~ Temp + Wind + Solar.R, data = airquality)

### façon tidyverse
firstreg <- airquality %>% 
  lm(Ozone ~ Temp + Wind + Solar.R, data = .) # le point fait référence aux données du pipe
```
...
firstreg %>% 
  ...
  geom_errorbar(aes(ymin = estimate - std.error, 
                    ymax = estimate + std.error), whidth = 0.1)+
...
augment ajoute les valeurs estimées à notre data.frame. Cela nous permet, par exemple, de voir la régression de façon visuelle (sur deux variables uniquement) en plottant les points intiaux et les points estimés. On va faire cela par étapes, et pour la relation Ozone ~ Temp
...
reg %>%  augment()
# colle le résultat de la régression aux données
```
...
```{r}
plot1 + geom_line(aes(x = Temp, y = .fitted), color = "green") # trace la droite de régression
```
...
```{r}
# résidus = distance entre les points et la ligne
plot1+
...
# même résultat mais c'est le plot qui fait la régression pour nous. 
```
...
df %>% 
  ...
  tidy()
# Le coefficient est petit mais positif. La p.value est très petite donc le 
# coefficient est très significatif, plus on est riche plus on vit longtemps.
```
...
# La régression n'est pas bien faite car la relation est logarithmique et on a
# fait une relation linéaire. 
```
...
df %>% 
  group_by(year) %>% 
  summarise(lm(lifeExp ~ gdpPercap, data = .))
# ne marche pas car summarise prend un jeu de données et retourne une valeur, et
# lm retourne un objet complexe. On va donc utiliser une fonction plus puissante
# que summarise. 
```
...
# On voit qu'en Europe et en Amérique, le PIB a de moins en moins d'impact sur 
# l'espérance de vie. Attention, un point sur ce graphique n'est pas une donnée
# mais un coefficient de régression. 
```
...
df %>% 
  ...
  geom_point()+
  facet_wrap(~continent, scales = "free")+
  geom_hline(yintercept = 12000, color = "green", linetype ="dotted")
