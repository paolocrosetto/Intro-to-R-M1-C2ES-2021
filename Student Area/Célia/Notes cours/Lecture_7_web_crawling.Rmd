...
df %>% 
  ggplot(aes(y = reorder(Country), x = GDP))+
  geom_col()
...
# clean the data
df %>% 
  pivot_longer(cols = -Season, names_to = "key")
...
dates <- rstudio %>% 
  html_elements("span:nth-child(1)") %>% 
  html_text()
...
```{r}
df <- tibble(dates, auteurs, titres, summary)
```
...
imdb <- tibble(titres, dates, étoiles)
