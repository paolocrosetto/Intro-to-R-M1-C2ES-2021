...
usa %>% 
  html_elements("p")
#on selectionne sur la page les élements htlm correctements intutulés (paragraphe p, lien a, ...) et renvoit le code html de ces différents élements
```
...
## Attention, ce ne sont que des caractères
```
...
df <- df %>% 
  mutate(GDP = str_replace(`GDP per capita (Int$)[1]`, ",", ""), GDP = as.numeric(GDP))
...
# les épisodes sont sur la première ligne du tableau, on va les mettre sur la première colonne!
...
- faire un histogramme des décénies les plus répresentées
...
clean <- imdb %>%
  mutate(date = str_replace(date, "\\(", ""),## ( est un paramètre spécial donc il faut mettre \\
         date = str_replace(date, "\\)", ""),
         date = as.numeric(date)) %>% 
  mutate(decennie = floor(date/10)*10) %>% ## floor permet de garder la partie entière de la division
  group_by(decennie) %>% 
  count()
...
clean %>% 
  ...
  theme_minimal() +
  labs(title = "Nombre de films dans le top 250 IMDB par décénnie")
...
Rcrawler(Website = "https://blog.rstudio.com/", 
         crawlUrlfilter = "/page/[0-9]{1-2}",  #expression régulière
...
```{r}
df <- read_delim("blog.rstudio.com")
```
