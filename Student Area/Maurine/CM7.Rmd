...
# il trouve les paragraphes présent dans la page
```
...
# il trouve les liens présent dans la page
```
...
# il trouve la table présent dans la page
```
...
# le tableau existe déjà, on veut juste le récuperer.
# Il y a un problème, toutes les données sont en charactères... Il faut donc modifier le type de la données.
```
...
############ récuperer ce que le prof a fait !! 
```
...
- faire un histogramme des décénnies les plus répresentées
...
clean %>% 
  ...
  theme_minimal() +
  labs(title = "Number of movies in the IMBD TP250 by")

#################" voir pourquoi cela ne fonctionne pas !!
```
...
```
# La réponse 200 correspond au fait que tout fonctionne comme il faut !
...
Rcrawler(Website = "https://blog.rstudio.com/",
         ...
         MaxDepth = 100,
         ManyPerPattern = T, # un ou plusieurs éléments : oui donc T sinon F
