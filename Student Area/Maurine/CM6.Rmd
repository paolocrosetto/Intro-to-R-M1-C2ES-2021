...
```{r ex1}
## Une regression s'écrit Y = k + b1*v1 + b2*V2 + ...
# En R il faut utiliser un tilde ! 
...
```{r}
tidy(firstreg) # on obtient un dataframe
```
...
```{r}
firstreg %>% 
  tidy() %>% # affiche le tableau tidy
  filter(term != "(Intercept)") %>% # on filtre l'intercept
  ggplot(aes(x = term, y = estimate)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = estimate - std.error,
                    ymax = estimate + std.error), width = 0.1) +
  geom_hline(yintercept = 0, color = "red") +
  coord_flip() + # permet de tourner le graph
  theme_minimal() # on met un thème au graph qui le rend plus joli
```
...
```{r}
thirdreg <- lm(Ozone ~Temp, data = airquality)
...
```{r}
plot1 +
  geom_line(aes(x = Temp, y = .fitted), color = "purple") +
  theme_light()
```
...
plot1 +
  geom_line(aes(x = Temp, y = .fitted), color = "purple") +
  geom_segment(aes(x = Temp, xend = Temp, y = Ozone, yend = .fitted)) + # si on veut les points sur les segments il aurait fallu faire l'inverse ! D'abord afficher les segments puis les points par la suite.
  theme_light()
...
airquality %>% 
  ggplot(aes(Wind, Ozone)) +
  geom_point() +
  geom_smooth(method = "lm", color = "purple") # regression linéaire comme faite ci dessus
```
...
# les données sont significatives. En effet la p.value est très petite ce qui signifie que ces deux variables sont fortement corrélées = relation plus que positive ! 
```
...
```{r}
df %>% 
  group_by(year) %>% # grouper par année ne fonctionne pas donc il faut faire autre chose.....
  summarise(m = lm(lifeExp ~ gdpPercap, data = .)) # ne fonctionne pas non plus.....
...
year_cont <- df %>% 
  ...
  group_modify(~tidy(lm(lifeExp ~ gdpPercap, data = .))) # ne marche pas, donc il faut rajouter tidy pour avoir un data frame
year_cont
```
...
pop_cont <- df %>% 
  group_by(continent) %>% 
  group_modify(~tidy(lm(pop ~ year, data = .))) # ne marche pas, donc il faut rajouter tidy pour avoir un data frame
pop_cont
```
...
df %>% 
  group_by(continent) %>% 
  summarise(correlation = cor.test(.$lifeExp, .$gdpPercap)) # avec les commandes .$ on spécifie la base de données où on doit prendre les variables
```
...
df %>% 
  group_by(continent) %>% 
  group_modify(~tidy(cor.test(.$lifeExp, .$gdpPercap))) # avec les commandes .$ on spécifie la base de données où on doit prendre les variables
```
...
df %>% 
  ...
  group_modify(~tidy(cor.test(df$lifeExp, df$gdpPercap))) %>% # avec les commandes .$ on spécifie la base de données où on doit prendre les variables
