...
# étoiles
etoiles <- imdb %>% html_element(("strong")) %>% html_text()

#3. coller les vecteurs ensemble avec "tibble()"
Fiche<- tibble(titres,date,etoiles)

#Cb de films dans le top 250 par decennie ?
clean <- imdb %>% 
  mutate(dates=str_replace(dates,"\\(",""),
         dates=str_replace(dates,"\\)",""),
         dates=as.numeric(dates)) %>% 
  mutate(decennie = floor(dates/10)*100)
group_by(decennie) %>% 
  count()
...
#1 ou 2 chiffres allant de 0 à 9 - donc [0-99]
