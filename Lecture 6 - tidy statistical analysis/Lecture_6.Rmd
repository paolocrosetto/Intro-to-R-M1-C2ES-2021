...
airquality %>% 
  ggplot(aes(Wind, Ozone))+
  geom_point()+
  geom_smooth(method = "lm") ## cela lance une régression linéaire comme celle faite ci-dessus
```
...
df %>% 
  group_by(continent, year) %>% 
  group_modify(~tidy(t.test(.$gdpPercap, mu = 12000, alternative = "less"))) %>% 
