...
=======
>>>>>>> f2ce5b9f3f73f7eeb70e3bdaec3359d0499726d1
#install.packages("rvest")
library(tidyverse)
...
<<<<<<< HEAD
df %>%
  select(State, croissance=`Percent change, 2010–2019[note 1]`)%>% # probleme croissance est un caractère
...
<<<<<<< HEAD
df2 <- read_html("https://en.wikipedia.org/wiki/List_of_African_countries_by_GDP_(PPP)_per_capita" ) %>%
  html_element("table") %>%
  html_table
...
#extract the data
<<<<<<< HEAD
df3 <- read_html("https://en.wikipedia.org/wiki/Game_of_Thrones#Viewership" ) 
...
dates <- rstudio%>%
  html_elements(".article-list span:nth-child(1)")%>%
  html_text()
=======
<<<<<<< HEAD
...
```{r}
<<<<<<< HEAD
df6 <- tibble(dates, titres, auteurs)
=======
<<<<<<< HEAD
...
# étoiles
étoiles <- imdb %>% html_elements("strong")%>% html_text()
...
#3. coller les vecteurs ensemble avec "tibble()"
imdb <- tibble(titres, date, étoiles)
imdb
View(imdb)
#4. nettoyage des données
<<<<<<< HEAD
imdb %>%
  mutate(dates=str_replace(date, "\\(",""),
...
```{r, message = FALSE}
 Rcrawler(Website = "https://blog.rstudio.com/", 
          ...
          MaxDepth = 100, 
<<<<<<< HEAD
          ManyPerPattern = T, # true car il y a plusieurs éléments pour chaque patterns
=======
          ManyPerPattern = T, 
>>>>>>> f2ce5b9f3f73f7eeb70e3bdaec3359d0499726d1
